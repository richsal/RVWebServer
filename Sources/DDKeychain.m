...
+ (NSString *)passwordForHTTPServerWithUsername:(NSString *)username
{
	...
	const char *service = [@"ProVideoPlayer2" UTF8String];
...
+ (BOOL)setPasswordForHTTPServer:(NSString *)password withUsername:(NSString *)username
{
	const char *service = [@"ProVideoPlayer2" UTF8String];
	const char *account = [username UTF8String];
	const char *kind    = [@"ProVideoPlayer2 password" UTF8String];
...
+ (void)createNewIdentity
{
    ...
    importParameters.keyUsage = NULL; //CSSM_KEYUSE_ANY;
    importParameters.keyAttributes = NULL; //CSSM_KEYATTR_SENSITIVE | CSSM_KEYATTR_EXTRACTABLE;
	...
	SecItemImport((__bridge CFDataRef)certData,  // CFDataRef importedData
...
//	HTTPLogVerbose(@"SecExternalFormat: %@", [DDKeychain stringForSecExternalFormat:inputFormat]);
//	HTTPLogVerbose(@"SecExternalItemType: %@", [DDKeychain stringForSecExternalItemType:itemType]);
//
//	HTTPLogVerbose(@"outItems: %@", (__bridge NSArray *)outItems);
