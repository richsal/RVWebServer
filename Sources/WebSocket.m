...
//	HTTPLogTrace();
	...
	if ((self = [super init]))
	{
		...
		pingBlocks = [[NSMutableArray alloc] init];
	}
	return self;
}
...
- (void) checkForTimedOutPingBlocks {
	//NSLog(@"%@ checking for timed out ping blocks...",self);
	// should be called from the websocketQueue on a timer...
	NSDate *now = [NSDate date];
	for(int i=0;i<pingBlocks.count;i++) {
		NSDictionary *d = pingBlocks[i];
		NSDate *timeout = d[@"timeout"];
		if([now isGreaterThanOrEqualTo:timeout]) {
			//NSLog(@"\t...%@ found timed out block!",self);
			void (^block)(void) = d[@"block"];
			if(block) dispatch_async(dispatch_get_main_queue(), block);
			[pingBlocks removeObjectAtIndex:i--];
		}
	}
}

- (void)sendPingData:(NSData *)msg withTimeout:(NSTimeInterval)timeout timeoutBlock:(void (^)(void))block {
//	HTTPLogTrace();
	
	NSMutableData *data = nil;
	
	//TODO: lookup whether the older protocols use the same ping pong procedure
	if (isRFC6455) {
	...
	if(data) {
		BOOL validTimeoutBlock = (timeout > 0);
		if(validTimeoutBlock) {
			dispatch_async(websocketQueue, ^{
				[self->pingBlocks addObject:@{@"block":block?:^{},
										@"timeout":[NSDate dateWithTimeIntervalSinceNow:timeout]}];
				//NSLog(@"%@ added to pingBlock array (%ld)",self,pingBlocks.count);
			});
		}
		[asyncSocket writeData:data withTimeout:TIMEOUT_NONE tag:0];
		if(validTimeoutBlock) {
			dispatch_after( dispatch_time(DISPATCH_TIME_NOW,(int64_t)(timeout * NSEC_PER_SEC)) , websocketQueue, ^{
				[self checkForTimedOutPingBlocks];
			});
		}
	}
}

- (void) respondToClose:(NSData *)msgData {
    //TODO: lookup whether the older protocols use the same close procedure
    if(isRFC6455) {
        NSMutableData *data = nil;
        NSUInteger length = msgData.length;
		UInt8 prefix[2] = { 0, 0 };
		prefix[0] = 0x80 | WS_OP_CONNECTION_CLOSE;
		if (length <= 125)
		{
			data = [NSMutableData dataWithCapacity:(length + 2)];
			[data appendBytes: prefix length:1];
...
- (void) respondToPing:(NSData *)msgData {
	//NSLog(@"%@ responding to ping %@",self,msgData);
    //TODO: lookup whether the older protocols use the same ping pong procedure
    if(isRFC6455) {
        NSMutableData *data = nil;
        NSUInteger length = msgData.length;
		UInt8 prefix[2] = { 0, 0 };
		prefix[0] = 0x80 | WS_OP_PONG;
		if (length <= 125)
		{
			data = [NSMutableData dataWithCapacity:(length + 2)];
			[data appendBytes: prefix length:1];
...
//	HTTPLogTrace();
	...
	//NSLog(@"%@ tag: %ld readData: %@ ",self,tag,data);
	...
	else if (tag == TAG_MSG_WITH_LENGTH)
	{
		...
		if (nextOpCode == WS_OP_TEXT_FRAME)
		{
            ...
            NSLog(@"need to handle continuation frames");
        }
        else if(nextOpCode == WS_OP_CONNECTION_CLOSE) {
            [self respondToClose:data];
        }
        else if(nextOpCode == WS_OP_PING) {
            [self respondToPing:data];
        }
        else if(nextOpCode == WS_OP_PONG) {
			// should already be on the websocketQueue...
			// this isn't bulletproof, if a pong comes back after the timeout, then things will go south
			if(pingBlocks.count) {
				[pingBlocks removeLastObject];
			} else {
				NSLog(@"got a pong with no matching block");
			}
        }
		else
		{
            NSLog(@"Unknown op code: %lx",(unsigned long)nextOpCode);
			[self stop];
			return;
		}
